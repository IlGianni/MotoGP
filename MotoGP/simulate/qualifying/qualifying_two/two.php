<?php
    ...
    if($race['status'] == 'Qualifying_Two'){
        // Get the riders
        $sql = "-- Primi 10 piloti dalla sessione 'Pre_Qualifying'
        SELECT * 
        FROM rider 
        ...
        -- Primi 2 piloti dalla sessione 'Qualifying_One'
        SELECT *
        FROM rider 
        INNER JOIN Team ON rider.idTeam = team.idTeam 
        INNER JOIN Qualifying ON rider.idRider = qualifying.idRider
        ...
        // Update race status
        $sql = "UPDATE Race SET status = 'Sprint_Start' WHERE idRace = " . $race['idRace'];
