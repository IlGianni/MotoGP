    ...
    <?php
        ...
        if($race['idMeteo'] == NULL) {
            ...
            if(rand(0, 100) < $track['percentage_rain'])
                $condition = 3;
            elseif(rand(0, 100) < $track['percentage_rain'] * 1.5)
                $condition = 2;
        ...
        if($race['status'] == 'Pre_Qualifying') {
            ...
            if(!$result)
                die("Error: " . mysqli_error($conn));
            // Pre-Qualifying
            require_once '../simulate/qualifying/pre_qualifying/pre3.php';
            ...
            // Qualifying One
            require_once '../simulate/qualifying/qualifying_one/one3.php';
            $sql = "SELECT rider.name, surname, number, color, lap_time, rider.idRider FROM rider 
                INNER JOIN team ON rider.idTeam = team.idTeam 
                INNER JOIN qualifying ON rider.idRider = qualifying.idRider 
                WHERE qualifying.idRace = " . $race['idRace'] . 
            ...
            // Qualifying Two
            require_once '../simulate/qualifying/qualifying_two/two3.php';
            $sql = "SELECT rider.name, surname, number, color, lap_time, rider.idRider FROM rider 
                INNER JOIN team ON rider.idTeam = team.idTeam 
                INNER JOIN qualifying ON rider.idRider = qualifying.idRider 
                WHERE qualifying.idRace = " . $race['idRace'] . 
            ...
            if(strpos($race['status'], 'Sprint') !== false) 
                require_once '../simulate/race/sprint/sprint3.php';
            else
                require_once '../simulate/race/full_race/race3.php';
            ...
            $sql = "SELECT rider.name, surname, number, color, standings.points, rider.idRider FROM rider 
                ...
                AND Standings.idCategory = 3
                ORDER BY Standings.points DESC";    
                
            // Label for session
            $session = 'Standings';
            echo "<label class='session'>Standings</label>";
        } elseif(strpos($race['status'], 'Finished3') !== false) {
            $sql = "UPDATE Race SET status = 'Pre_Qualifying', category = 'Moto2' WHERE idRace = " . $race['idRace'];
            ...
            if(!$update) {
                ...
                echo "<script>window.location.href='Moto2.php';</script>";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<div class='riderBox'>";
                ...
                if(strpos($race['status'], 'End') !== false) {
                    ...
                    $sql = "UPDATE Standings SET points = points + " . $row_position['points'] . " WHERE idRider = " . $row['idRider'] . " AND idCategory = 3 AND idSeason = " . $race['idSeason'];
                    $update = mysqli_query($conn, $sql);
                    if(!$update)
                        die("Error: " . mysqli_error($conn));
                }
        ...
        if(strpos($race['status'], 'Standings') !== false) {
            $sql = "UPDATE Race SET status = 'Finished3' WHERE idRace = " . $race['idRace'];
            $update = mysqli_query($conn, $sql);
            if(!$update)
                die("Error: " . mysqli_error($conn));
