...
<body>
    ...
    <?php
        ...
        if($race['status'] == 'Pre_Qualifying') {
            ...
            // Pre-Qualifying
            require_once '../simulate/qualifying/pre_qualifying/pre.php';
            ...
            // Qualifying One
            require_once '../simulate/qualifying/qualifying_one/one.php';
            $sql = "SELECT rider.name, surname, number, color, lap_time, rider.idRider FROM rider 
                INNER JOIN team ON rider.idTeam = team.idTeam 
                INNER JOIN qualifying ON rider.idRider = qualifying.idRider 
                WHERE qualifying.idRace = " . $race['idRace'] . 
            ...
            // Qualifying Two
            require_once '../simulate/qualifying/qualifying_two/two.php';
            $sql = "SELECT rider.name, surname, number, color, lap_time, rider.idRider FROM rider 
                INNER JOIN team ON rider.idTeam = team.idTeam 
                INNER JOIN qualifying ON rider.idRider = qualifying.idRider 
                WHERE qualifying.idRace = " . $race['idRace'] . 
            ...
            if(strpos($race['status'], 'Sprint') !== false) 
                require_once '../simulate/race/sprint/sprint.php';
            else
                require_once '../simulate/race/full_race/race.php';
            ...
            if(strpos($race['status'], 'Sprint_Lap') !== false) {
                if($lap == round($track['race_distance']/2) - 2)
                    echo "<label class='session'>LAST LAP</label>";
                else    
                echo "<label class='session'>" . round($track['race_distance']*$category['percentage_race_distance']/100/2) - ($lap+1) . " LAPS TO GO</label>";
                $session = 'Sprint Lap';
            } else {
                if($lap == round($track['race_distance']) - 2)
                    echo "<label class='session'>LAST LAP</label>";
                else    
                echo "<label class='session'>" . round($track['race_distance']*$category['percentage_race_distance']/100) - ($lap+1) . " LAPS TO GO</label>";
                $session = 'Race Lap';
            }
            ...
            if(strpos($race['status'], 'Sprint') !== false) {
                require_once '../simulate/race/sprint/sprint.php';
                $lap = round($track['race_distance']/2);
            } else {
                require_once '../simulate/race/full_race/race.php';
                $lap = round($track['race_distance']);
            }
            $sql = "SELECT rider.name, surname, number, color, total_time, rider.idRider FROM rider 
                INNER JOIN team ON rider.idTeam = team.idTeam 
                INNER JOIN race_results ON rider.idRider = race_results.idRider 
            ...
            $sql = "SELECT rider.name, surname, number, color, standings.points, rider.idRider FROM rider 
                ...
                AND Standings.idCategory = 1
                ORDER BY Standings.points DESC";    
                
            // Label for session
            $session = 'Standings';
            echo "<label class='session'>Standings</label>";
        } elseif(strpos($race['status'], 'Finished') !== false) {
            // check if there is another track
            $sql = "SELECT * FROM track WHERE idTrack = " . $race['idTrack'] + 1;
            $result = mysqli_query($conn, $sql);
            if(!$result)
                die("Error: " . mysqli_error($conn));
            if(mysqli_num_rows($result) == 0) {
                $sql = "SELECT * FROM season WHERE idSeason = " . $race['idSeason'];
                $result = mysqli_query($conn, $sql);
                if(!$result)
                    die("Error: " . mysqli_error($conn));
                $season = mysqli_fetch_assoc($result);
                $sql = "INSERT INTO season (year) VALUES (" . ($season['year'] + 1) . ")";
                $insert = mysqli_query($conn, $sql);
                if(!$insert)
                    die("Error: " . mysqli_error($conn));
                $sql = "INSERT INTO Race (idTrack, idSeason, status, category) VALUES (1, " . $race['idSeason'] + 1 . ", 'Pre_Qualifying', 'Moto3')";
                $insert = mysqli_query($conn, $sql);
                if(!$insert)
                    die("Error: " . mysqli_error($conn));
            ...
            $sql = "INSERT INTO Race (idTrack, idSeason, status, category) VALUES (" . $race['idTrack'] + 1 . ", " . $race['idSeason'] . ", 'Pre_Qualifying', 'Moto3')";
            $insert = mysqli_query($conn, $sql);
                ...
                if(!$insert) {
                    ...
                    if(!$delete)
                        die("Error: " . mysqli_error($conn));
                    $sql = "DELETE FROM qualifying";
                    $delete = mysqli_query($conn, $sql);
                    if(!$delete)
                        die("Error: " . mysqli_error($conn));
                    echo "<script>window.location.href='standings/standings.php';</script>";
                    exit(); // Ensure that script stops executing after echoing JavaScript
                }
            }
        }
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo "<div class='riderBox'>";
                ...
                if(strpos($race['status'], 'End') !== false) {
                    ...
                    $sql = "UPDATE Standings SET points = points + " . $row_position['points'] . " WHERE idRider = " . $row['idRider'] . " AND idCategory = 1 AND idSeason = " . $race['idSeason'];
                    $update = mysqli_query($conn, $sql);
                    if(!$update)
                        die("Error: " . mysqli_error($conn));
                }
        ...
        if(strpos($race['status'], 'Standings') !== false) {
            $sql = "UPDATE Race SET status = 'Finished' WHERE idRace = " . $race['idRace'];
            $update = mysqli_query($conn, $sql);
            if(!$update)
                die("Error: " . mysqli_error($conn));
